...
export default function Resources() {
  ...
  const load = () => Promise.all([workersApi.getAll(), equipmentApi.getAll()])
  ...
  const openWorker = (item = null) => {
    setEditItem(item)
    setForm(item ? { name: item.name, skill: item.skill, status: item.status }
                 : { name: '', skill: '', status: true })
    setModal('worker')
  }
  const openEquip = (item = null) => {
    setEditItem(item)
    setForm(item ? { name: item.name, type: item.type, status: item.status }
  ...
  return (
    <div>
      ...
      {/* Tabs */}
      <div style={{ padding: '16px 28px 0', display: 'flex', gap: 4, borderBottom: '1px solid var(--border-solid)' }}>
        {[['workers', '👷 Работники', workers.length], ['equip', '⚙️ Оборудование', equipment.length]].map(([key, label, cnt]) => (
          <button key={key} onClick={() => setTab(key)} style={{
            padding: '8px 18px', border: 'none', cursor: 'pointer',
            fontFamily: 'Outfit, sans-serif', fontSize: 13, fontWeight: tab === key ? 600 : 400,
            color: tab === key ? 'var(--primary-dark)' : 'var(--text-muted)',
            background: 'transparent',
            borderBottom: `2px solid ${tab === key ? 'var(--primary)' : 'transparent'}`,
            marginBottom: -1,
          }}>
            {label} <span style={{ ...ui.badge(tab === key ? 'green' : 'gray'), fontSize: 11, marginLeft: 4 }}>{cnt}</span>
          </button>
        ))}
      </div>
        ...
        <div style={{ ...ui.card, overflow: 'hidden' }}>
          {loading ? (
            ...
            <table style={ui.table}>
              <thead>
                <tr>{['№', 'ФИО', 'Специализация', 'Статус', 'Действия'].map(h => <th key={h} style={ui.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {workers.map(w => (
                  <tr key={w.id}
                  ...
                  >
                    ...
                    <td style={ui.td}><span style={ui.badge('green')}>{w.skill || '—'}</span></td>
                    <td style={ui.td}>
            ...
            <table style={ui.table}>
              <thead>
                <tr>{['№', 'Наименование', 'Тип', 'Статус', 'Действия'].map(h => <th key={h} style={ui.th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {equipment.map(e => (
                  <tr key={e.id}
                  ...
                  >
                    ...
                    <td style={ui.td}><span style={ui.badge('green')}>{e.type}</span></td>
                    <td style={ui.td}>
      ...
      {modal === 'worker' && (
        <Modal title={editItem ? 'Редактировать работника' : 'Новый работник'} onClose={() => setModal(null)}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 16, borderTop: '1px solid var(--border-solid)' }}>
      ...
      {modal === 'equip' && (
        <Modal title={editItem ? 'Редактировать станок' : 'Новый станок'} onClose={() => setModal(null)}>
          <div style={{ marginBottom: 14 }}>
